...
class ASRProviderBase(ABC):
    ...
    async def handle_voice_stop(self, conn, asr_audio_task: List[bytes]):
        ...
        try:
            ...
            # 🔧 修复ASR音频累积问题：处理完成后清空音频缓冲区
            self._clear_audio_buffers(conn)
    ...
    def _clear_audio_buffers(self, conn):
        """清空音频缓冲区，防止音频数据累积"""
        try:
            # 清空传统的音频列表
            if hasattr(conn, 'asr_audio'):
                conn.asr_audio.clear()
                logger.bind(tag=TAG).debug("已清空asr_audio缓冲区")
            
            if hasattr(conn, 'asr_audio_for_voiceprint'):
                conn.asr_audio_for_voiceprint.clear()
                logger.bind(tag=TAG).debug("已清空asr_audio_for_voiceprint缓冲区")
            
            # 清空新的音频管理器缓冲区
            if hasattr(conn, '_audio_manager'):
                conn._audio_manager.clear()
                logger.bind(tag=TAG).debug("已清空AudioBufferManager缓冲区")
            
            logger.bind(tag=TAG).info("🧹 ASR处理完成，音频缓冲区已清空")
            
        except Exception as e:
            logger.bind(tag=TAG).warning(f"清空音频缓冲区时出错: {e}")
