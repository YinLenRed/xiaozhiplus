...
class UnifiedToolHandler:
    ...
    def get_functions(self) -> List[Dict[str, Any]]:
        """获取所有工具的函数描述"""
        # 如果工具处理器尚未完全初始化，返回空列表并记录日志
        if not self.finish_init:
            self.logger.warning("工具处理器尚未完全初始化，返回空函数列表")
            return []
    ...
    def has_tool(self, tool_name: str) -> bool:
        """检查是否有指定工具"""
        # 如果工具处理器尚未完全初始化，返回False
        if not self.finish_init:
            self.logger.warning(f"工具处理器尚未完全初始化，工具 {tool_name} 暂不可用")
            return False
    ...
    async def handle_llm_function_call(
        ...
        # 确保工具处理器已完全初始化
        if not self.finish_init:
            self.logger.info("工具处理器尚未完全初始化，等待初始化完成...")
            await self._initialize()
            self.logger.info("工具处理器初始化完成，继续处理函数调用")
        try:
            # 处理多函数调用
            ...
            # 🔍 调试日志：追踪unified_tool_handler返回
            if hasattr(result, 'action') and hasattr(result, 'response'):
                response_len = len(result.response) if result.response else 0
                self.logger.info(f"🎤 unified_tool_handler返回: action={result.action}, response长度={response_len}")
                if result.response:
                    self.logger.info(f"🎤 unified_tool_handler返回内容: {result.response[:50]}...")
