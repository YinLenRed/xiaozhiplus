...
class WebSocketServer:
    ...
    async def send_audio_to_device(self, device_id: str, audio_file_path: str, track_id: str, greeting_text: str = None) -> bool:
        ...
        try:
            ...
            # 关键修复：增加连接等待和重试机制，解决时序问题
            connection = None
            max_retries = 6  # 最多重试6次，每次等待1秒，总共6秒
            ...
            for retry in range(max_retries):
                ...
                if retry < max_retries - 1:  # 不是最后一次重试
                    self.logger.bind(tag=TAG).info(f"设备连接未就绪，等待重连: {device_id} (重试 {retry+1}/{max_retries})")
            ...
            if connection and connection.websocket:
                ...
                if connection.tts and hasattr(connection.tts, 'tts_audio_queue'):
                    ...
                    # 3. 设置任务完成标志，让最后一个LAST消息触发stop
                    connection.llm_finish_task = True
                    self.logger.bind(tag=TAG).info("✅ 主动问候：设置任务完成标志（将触发预缓冲优化传输）")
                else:
                    # 降级方案：如果没有TTS队列，直接发送
