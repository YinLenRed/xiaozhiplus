    ...
    def _chat_internal(self, query, tool_call=False, depth=0, chat_session_id=None):
        ...
        if depth == 0:
            ...
            if self.client_abort:
                self.logger.bind(tag=TAG).info("🚫 检测到abort状态，跳过LLM处理")
                return None  # 直接返回，不处理LLM
            if self.tts is not None:
            ...
            if content is not None and len(content) > 0:
                if not tool_call_flag:
                    ...
                    if self.tts is not None:
                        ...
                        # 🛠️ 关键修复：跳过空内容，避免TTS处理空字符串
                        if not cleaned_content or not cleaned_content.strip():
                            self.logger.bind(tag=TAG).info(f"⏭️ 跳过空内容TTS消息: '{original_content}' 过滤后为空")
                            continue
