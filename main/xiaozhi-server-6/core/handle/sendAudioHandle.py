...
# 🚀 优化版音频播放 - WebSocket + 预缓冲优化
async def sendAudio(conn, audios, pre_buffer=True):
    # 🔧 注册当前任务以便abort时取消
    current_task = asyncio.current_task()
    if not hasattr(conn, '_audio_send_tasks'):
        conn._audio_send_tasks = set()
    conn._audio_send_tasks.add(current_task)
    
    # 🔧 确保任务清理
    def cleanup_task():
        if hasattr(conn, '_audio_send_tasks') and current_task in conn._audio_send_tasks:
            conn._audio_send_tasks.discard(current_task)
    ...
    # 🚀 WebSocket + 预缓冲优化 - 性能监控开始
    optimization_start_time = time.perf_counter()
    conn.logger.bind(tag=TAG).info(f"🎵 开始优化音频发送: {len(audios)}帧, pre_buffer={pre_buffer}")
    
    # 📊 获取性能监控器（如果可用）
    monitor = get_performance_monitor()
    track_id = getattr(conn, 'current_track_id', None) or getattr(conn, 'sentence_id', 'unknown')
    
    # 优化流控参数
    frame_duration = 60  # 帧时长（毫秒），匹配 Opus 编码
    optimized_frame_interval = 55  # 优化间隔：比60ms略快，提升响应速度
    ...
    # 🎯 智能预缓冲策略：根据音频长度动态调整
    if pre_buffer:
        # 动态预缓冲帧数：短音频多缓冲，长音频少缓冲
        if len(audios) <= 10:  # 短音频（<1秒）
            pre_buffer_frames = min(5, len(audios))  # 最多缓冲5帧
        elif len(audios) <= 30:  # 中等音频（1-3秒）
            pre_buffer_frames = min(4, len(audios))  # 缓冲4帧
        else:  # 长音频（>3秒）
            pre_buffer_frames = min(3, len(audios))  # 缓冲3帧
            
        conn.logger.bind(tag=TAG).info(f"🚀 智能预缓冲: {pre_buffer_frames}帧 (总帧数: {len(audios)})")
        ...
        for i in range(pre_buffer_frames):
            try:
                ...
                conn.logger.bind(tag=TAG).error(f"❌ 预缓冲发送失败: {ws_error}")
                break
        
        prebuffer_time = (time.perf_counter() - prebuffer_start) * 1000
        conn.logger.bind(tag=TAG).info(f"⚡ 预缓冲完成: {pre_buffer_frames}帧 用时{prebuffer_time:.1f}ms")
        
        # 📊 更新预缓冲指标
        if track_id != 'unknown':
            monitor.update_prebuffer(track_id, pre_buffer_frames, prebuffer_time)
        
        remaining_audios = audios[pre_buffer_frames:]
        # 调整播放位置，考虑已发送的预缓冲帧
        play_position = pre_buffer_frames * frame_duration
    else:
    ...
    for i, opus_packet in enumerate(remaining_audios):
        ...
        if delay > 0:
            # 限制最大延迟，避免过度等待
            actual_delay = min(delay, optimized_frame_interval / 1000)
            await asyncio.sleep(actual_delay)

        # 🚫 在发送前再次检查abort状态（关键修复）
        if conn.client_abort:
            conn.logger.bind(tag=TAG).info(f"⏹️ 准备发送时检测到abort，立即停止，已发送{sent_frames}帧")
            break
        ...
        try:
            ...
            # 🔧 性能日志：每10帧记录一次，包含传输速度
            if sent_frames % 10 == 0:
                elapsed_time = (time.perf_counter() - optimization_start_time) * 1000
                avg_speed = sent_frames / elapsed_time * 1000  # 帧/秒
                conn.logger.bind(tag=TAG).info(f"📊 传输进度: {sent_frames}/{len(audios)}帧, 速度: {avg_speed:.1f}帧/s")
                
                # 📊 更新进度监控
                if track_id != 'unknown':
                    monitor.update_progress(track_id, sent_frames, 0)
        ...
        # 使用优化的帧间隔
        play_position += optimized_frame_interval
    
    # 🚀 传输完成统计
    total_time = (time.perf_counter() - optimization_start_time) * 1000
    if sent_frames > 0:
        avg_speed = sent_frames / (total_time / 1000)  # 帧/秒
        theoretical_duration = len(audios) * frame_duration  # 理论播放时长
        speed_ratio = total_time / theoretical_duration  # 传输时间/播放时间比例
        
        conn.logger.bind(tag=TAG).info(
            f"🎵 优化音频发送完成: {sent_frames}/{len(audios)}帧, "
            f"用时{total_time:.1f}ms, 平均{avg_speed:.1f}帧/s, "
            f"传输效率: {speed_ratio:.2f}x (越小越好)"
        )
...
async def send_tts_message(conn, state, text=None):
    ...
    if state == "start":
        ...
        conn.tts_actually_started = True  # 🔧 标记TTS真正开始播放
        conn.logger.bind(tag=TAG).info(f"🎯 TTS开始播放，设置说话状态: client_is_speaking = True, tts_actually_started = True")
        ...
        if not (hasattr(conn, 'just_sent_speaking_status') and conn.just_sent_speaking_status):
            ...
            conn.logger.bind(tag=TAG).info(f"📱 TTS start发送屏幕状态更新: speaking")
        else:
        ...
        conn.logger.bind(tag=TAG).info(f"🎯 TTS播放结束，清除说话状态: client_is_speaking = False, tts_actually_started = False")
