...
from core.handle import sendAudioHandle  # 导入TTS播放完成事件处理
...
class UnifiedEventService:
    ...
    def __init__(self, mqtt_client: MQTTClient = None):
        ...
        # 🔧 关键修复：使用与webhook相同的TTS初始化方式
        try:
            # 优先尝试使用全局TTS实例（与webhook共享）
            ...
            self.tts_provider = modules.get("tts")
            if self.tts_provider:
                logger.bind(tag=TAG).info("✅ UnifiedEventService TTS提供器初始化成功（共享实例）")
            else:
                raise Exception("TTS模块初始化返回None")
                
        except Exception as e:
            logger.bind(tag=TAG).error(f"❌ UnifiedEventService TTS提供器初始化失败: {e}")
            logger.bind(tag=TAG).warning("🔧 尝试使用默认TTS配置...")
            # 备用方案：使用简单的TTS配置（避免WebSocket URL问题）
            try:
                # 检查是否有EdgeTTS可用（不需要WebSocket）
                try:
                    ...
                    self.tts_provider = EdgeTTSProvider(edge_config, False)
                    logger.bind(tag=TAG).info("✅ 使用EdgeTTS作为备用TTS提供器")
                except ImportError:
                    # EdgeTTS不可用，使用火山引擎但跳过WebSocket问题
                    logger.bind(tag=TAG).warning("EdgeTTS不可用，TTS功能将受限")
                    self.tts_provider = None
                    
            except Exception as e2:
                logger.bind(tag=TAG).error(f"❌ 备用TTS配置也失败: {e2}")
                self.tts_provider = None
    ...
    async def _handle_device_event(self, event_data: Dict[str, Any], device_id: str, topic: str):
        ...
        try:
            ...
            if event_type == "EVT_SPEAK_DONE":
                ...
                logger.bind(tag=TAG).info(f"🎯 设备播放完成: {device_id}, track_id: {track_id}, status: {status}")
                
                # 🎯 新增：处理TTS播放完成事件（用于VAD启动时机控制）
                await self._handle_tts_speak_done_event(device_id, track_id, status)
...
def get_unified_event_service(mqtt_client: MQTTClient = None) -> UnifiedEventService:
    ...
    async def _handle_tts_speak_done_event(self, device_id: str, track_id: str, status: str):
        """处理TTS播放完成事件，用于VAD启动时机控制"""
        try:
            logger.bind(tag=TAG).info(f"🎵 处理TTS播放完成事件: {device_id}, track_id: {track_id}")
            
            # 查找对应设备的WebSocket连接
            connection = self._find_websocket_connection(device_id)
            
            if connection:
                logger.bind(tag=TAG).info(f"✅ 找到WebSocket连接: {device_id}")
                
                # 调用TTS播放完成事件处理器
                await sendAudioHandle.handle_speak_done_event(connection, track_id, status)
                
                logger.bind(tag=TAG).info(f"🎤 TTS播放完成事件已处理，VAD将启动聆听")
            else:
                logger.bind(tag=TAG).warning(f"⚠️ 未找到设备WebSocket连接: {device_id}")
                
        except Exception as e:
            logger.bind(tag=TAG).error(f"❌ 处理TTS播放完成事件失败: {e}")
    
    def _find_websocket_connection(self, device_id: str):
        """查找指定设备的WebSocket连接"""
        try:
            # 从全局WebSocket服务器查找活跃连接
            ...
            if hasattr(self, 'server') and self.server:
                if hasattr(self.server, 'active_connections'):
                    for conn in self.server.active_connections:
                        if hasattr(conn, 'device_id') and conn.device_id == device_id:
                            return conn
            
            # 尝试从MQTT客户端获取服务器引用
            if self.mqtt_client and hasattr(self.mqtt_client, 'server') and self.mqtt_client.server:
                if hasattr(self.mqtt_client.server, 'active_connections'):
                    for conn in self.mqtt_client.server.active_connections:
                        if hasattr(conn, 'device_id') and conn.device_id == device_id:
                            return conn
            
            logger.bind(tag=TAG).debug(f"🔍 在活跃连接中未找到设备: {device_id}")
            return None
            
        except Exception as e:
            logger.bind(tag=TAG).error(f"❌ 查找WebSocket连接失败: {e}")
            return None
