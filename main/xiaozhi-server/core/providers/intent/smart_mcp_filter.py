...
class SmartMCPFilter:
    ...
    def __init__(self):
        ...
        # ⏰ 时间维度关键词（用于区分实时vs静态信息）
        self.time_dimension_keywords = [
            # 明确的时间指示词
            "今天", "昨天", "明天", "今年", "去年", "明年", "本周", "上周", "下周",
            "这个月", "上个月", "下个月", "今日", "昨日", "明日", "当天", "当日",
            "近期", "最近", "刚刚", "现在", "当前", "目前", "即时", "实时",
            
            # 时效性表达
            "最新", "新出", "新发布", "新推出", "新发现", "新研究", "新开发",
            "刚出", "刚发布", "刚推出", "刚发现", "刚公布", "刚宣布",
            "最新发布", "最新推出", "最新发现", "最新研究", "最新开发",
            
            # 变化性表达
            "更新", "变化", "改变", "发展", "进展", "动态", "趋势",
            "最新动态", "最新进展", "最新变化", "最新发展",
            
            # 特定时间段
            "2024", "2025", "本年度", "今年度", "当年", "这一年",
            "这几天", "这段时间", "这个时期", "近几天", "近几周", "近几个月"
        ]
        ...
        self.health_keywords = [
            # 疾病症状
            "高血压", "糖尿病", "关节炎", "膝盖疼", "腰背痛", "头痛", "胸痛",
            "发烧", "咳嗽", "感冒", "失眠", "便秘", "腹泻", "心脏病",
            "冠心病", "脑梗", "中风", "哮喘", "胃病", "肝病", "肾病",
            "骨质疏松", "风湿", "痛风", "甲状腺", "血脂", "血糖", "血压",
            
            # 身体部位不适
            "疼", "痛", "酸", "胀", "麻", "肿", "痒", "晕", "乏力", "没力气",
            "走路", "摔倒", "跌倒", "头晕", "胸闷", "气短", "心慌",
            
            # 用药相关
            "药", "吃药", "服药", "用药", "剂量", "副作用", "药物", "治疗",
            "怎么吃", "什么时候吃", "注意事项", "禁忌", "药效",
            
            # 健康咨询
            "应该注意什么", "怎么办", "如何治疗", "如何缓解", "如何预防",
            "注意什么", "饮食", "运动", "生活习惯", "保健", "养生"
        ]
    ...
    def should_enable_mcp(self, user_input: str) -> Dict[str, Any]:
        ...
        # 5. 🏥 检查健康医疗咨询需求
        health_matched = self._check_keywords(clean_input, self.health_keywords)
        if health_matched:
            result.update({
                "enabled": True,
                "reason": "health_consultation",
                "keywords_matched": health_matched,
                "confidence": 0.9
            })
            logger.bind(tag=TAG).info(f"🏥 启用MCP: 检测到健康医疗咨询需求 {health_matched}")
            return result
        
        # 6. ⏰ 检查时间维度的信息查询（时效性需求）
        time_matched = self._check_keywords(clean_input, self.time_dimension_keywords)
        
        # 7. 🤔 模糊判断：包含疑问词且可能需要外部信息
        if self._is_information_query(clean_input):
            # 检查是否有时间维度，如果有则优先启用MCP
            if time_matched:
                result.update({
                    "enabled": True,
                    "reason": "time_sensitive_query",
                    "keywords_matched": time_matched + ["疑问词+信息需求"],
                    "confidence": 0.95
                })
                logger.bind(tag=TAG).info(f"⏰ 启用MCP: 检测到时效性信息查询需求 {time_matched}")
            else:
                result.update({
                    "enabled": False,  # 🔧 无时间维度的一般信息查询优先使用LLM
                    "reason": "general_information_query_prefer_llm",
                    "keywords_matched": ["疑问词+信息需求"],
                    "confidence": 0.3
                })
                logger.bind(tag=TAG).info(f"💭 不启用MCP: 检测到一般信息查询，优先使用LLM")
            return result
    ...
    def get_filtered_mcp_tools(self, all_mcp_tools: List[Dict], filter_result: Dict[str, Any]) -> List[Dict]:
        ...
        if reason == "search_request":
            ...
            # 实时需求：返回实时信息工具
            return self._filter_tools_by_name(all_mcp_tools, ["weather", "news", "stock", "realtime", "search", "web", "bailian"])
        
        elif reason == "entertainment_request":
            # 娱乐需求：返回娱乐相关工具
            return self._filter_tools_by_name(all_mcp_tools, ["movie", "music", "entertainment"])
        
        elif reason == "health_consultation":
            # 健康咨询需求：返回搜索工具（用于获取最新医疗信息）
            return self._filter_tools_by_name(all_mcp_tools, ["search", "web", "bailian"])
        
        elif reason == "time_sensitive_query":
            # 时效性查询需求：返回搜索工具（用于获取最新信息）
            return self._filter_tools_by_name(all_mcp_tools, ["search", "web", "bailian"])
